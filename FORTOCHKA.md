...
- [~] `zhelezo` tier 0: IA-32 user-mode interpreter (int + x87 + the SSE1 RTW
      ...
      50-MIPS budget. **x87 FPU** LIVE (f64 stack, load/store/arith/compare/
      transcendental/control subset; `used` determinism flag): `fpu_arith.s`,
      `fpu_cmp.s`, corpus `fpu.exe` (real float PE → 42) pass. **SSE1** LIVE
      (8 XMM + MXCSR; mov/arith/logic/cvt/compare/shuffle/mxcsr scalar+packed
      single): `sse_scalar.s`, `sse_packed.s` pass. Integer + x87 + SSE1 = the
      full tier-0 ISA the plan scoped. Corpus driver: `tests/run.py` (MinGW as →
      flat bin → assert); `--no-cache` for A/B.
