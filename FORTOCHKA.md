...
- [~] `zhelezo` tier 0: IA-32 user-mode interpreter (int + x87 + the SSE1 RTW
      ...
      `fpu_cmp.s`, corpus `fpu.exe` (real float PE → 42) pass. Still missing:
      SSE1 (RTW uses a little). Corpus driver: `tests/run.py` (MinGW as → flat
      bin → assert); `--no-cache` for A/B.
